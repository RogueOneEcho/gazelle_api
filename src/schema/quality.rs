...
mod tests {
    ...
    fn deserialize_lossless() {
        let output: Quality = serde_json::from_str("\"Lossless\"").unwrap();
        assert_eq!(output, Quality::Lossless);
    }

    #[test]
    fn deserialize_24bit_lossless() {
        let output: Quality = serde_json::from_str("\"24bit Lossless\"").unwrap();
        assert_eq!(output, Quality::Lossless24);
    }

    #[test]
    fn deserialize_vbr() {
        let output: Quality = serde_json::from_str("\"V0 (VBR)\"").unwrap();
        assert_eq!(output, Quality::V0);
    }

    #[test]
    fn deserialize_numeric() {
        let output: Quality = serde_json::from_str("\"320\"").unwrap();
        assert_eq!(output, Quality::_320);
    }

    #[test]
    fn deserialize_q8x() {
        let output: Quality = serde_json::from_str("\"q8.x (VBR)\"").unwrap();
        assert_eq!(output, Quality::Q8x);
    }

    #[test]
    fn deserialize_dsd() {
        let output: Quality = serde_json::from_str("\"DSD128\"").unwrap();
        assert_eq!(output, Quality::DSD128);
    }

    #[test]
    fn deserialize_unknown() {
        let output: Quality = serde_json::from_str("\"Custom 448\"").unwrap();
        assert_eq!(output, Quality::Other("Custom 448".to_owned()));
    }

    #[test]
    fn deserialize_empty_string() {
        let output: Quality = serde_json::from_str("\"\"").unwrap();
        assert_eq!(output, Quality::Other(String::new()));
    }

    #[test]
    fn serialize_lossless() {
        let output = serde_json::to_string(&Quality::Lossless).unwrap();
        assert_eq!(output, "\"Lossless\"");
    }

    #[test]
    fn serialize_vbr() {
        let output = serde_json::to_string(&Quality::V0).unwrap();
        assert_eq!(output, "\"V0 (VBR)\"");
    }

    #[test]
    fn serialize_numeric() {
        let output = serde_json::to_string(&Quality::_320).unwrap();
    ...
    fn serialize_round_trip() {
        ...
        let quality: Quality = serde_json::from_str(original).unwrap();
