...
mod tests {
    ...
    fn deserialize_known() {
        let output: Media = json_from_str("\"CD\"").expect("should deserialize");
        assert_eq!(output, Media::CD);
    }

    #[test]
    fn deserialize_blu_ray() {
        let output: Media = json_from_str("\"Blu-Ray\"").expect("should deserialize");
        assert_eq!(output, Media::BluRay);
    }

    #[test]
    fn deserialize_bd() {
        let output: Media = json_from_str("\"BD\"").expect("should deserialize");
        assert_eq!(output, Media::BD);
    }

    #[test]
    fn deserialize_unknown() {
        let output: Media = json_from_str("\"MiniDisc\"").expect("should deserialize");
        assert_eq!(output, Media::Other("MiniDisc".to_owned()));
    }

    #[test]
    fn deserialize_empty_string() {
        let output: Media = json_from_str("\"\"").expect("should deserialize");
        assert_eq!(output, Media::Other(String::new()));
    }

    #[test]
    fn serialize_known() {
        let output = json_to_string(&Media::WEB).expect("should serialize");
        assert_eq!(output, "\"WEB\"");
    }

    #[test]
    fn serialize_blu_ray() {
        let output = json_to_string(&Media::BluRay).expect("should serialize");
    ...
    fn serialize_other() {
        let output =
            json_to_string(&Media::Other("MiniDisc".to_owned())).expect("should serialize");
    ...
    fn serialize_round_trip() {
        ...
        let media: Media = json_from_str(original).expect("should deserialize");
        let serialized = json_to_string(&media).expect("should serialize");
